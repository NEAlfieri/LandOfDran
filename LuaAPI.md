...
studs of where a rocket lands, fading with the distance to the nearest part of them, the shooter included.
...
Players have a Saved Bricks window (escape menu, see the `ClientSaveBricks` and `ClientLoadBricks`
events) over their **own** `Saves` folder: it lists every `.lod` and `.bls` file there with its brick count,
who saved it, when, and a picture. For an admin, Save asks the server for a copy of every brick, which the
server writes with the player's name and the time and the player's game adds a picture to. Anyone else's
game saves what it can see of the bricks on its own, without names, owners, lights, music, emitters, or
spawns, and the window says so. Load uploads one of those files for the server to load, which only admins
can do. `saveBuild` writes the time alone, so its saves list
